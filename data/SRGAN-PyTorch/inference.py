...
# torch.cuda.set_per_process_memory_fraction(1/2)
...
def main(args):
    ...
    sr_model.eval()
    save_path = '/workspace/omkar_projects/PyTorch-YOLOv3/data/Ball_SR/'
    data_path = '/workspace/omkar_projects/PyTorch-YOLOv3/data/Ball_dataset/'
    dirs = ['train/', 'valid/', 'test/']
    for dir in dirs:
        for image in glob.glob(data_path+dir+'*.jpg'):
            name = image.split('/')[-1]
            try:
                if os.path.exists(save_path+name)==False:
                    ...
                    cv2.imwrite(save_path+name, sr_image)

                    print(f"SR image save to `{save_path+name}`")
            except:
                pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_weights_path",
                        type=str,
                        default="/workspace/omkar_projects/PyTorch-YOLOv3/data/SRGAN-PyTorch/SRGAN_x4-ImageNet-8c4a7569.pth.tar",
